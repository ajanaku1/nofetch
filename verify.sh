...
assert.match(systemPrompt, /EMPTY ENVIRONMENT OVERRIDE:[\s\S]*Remove the empty override/);
assert.match(systemPrompt, /EXISTING VIRTUAL ENVIRONMENT:[\s\S]*-c "import \[module\]"/);
assert.match(systemPrompt, /RECOVERABLE GIT COMMIT:[\s\S]*Create a recovery branch/);
assert.match(systemPrompt, /UNQUOTED SPACED PATH:[\s\S]*Quote the complete supplied path/);
assert.match(systemPrompt, /PROVEN MISSING REQUIRED TOOL:[\s\S]*Request approval to acquire/);
assert.match(systemPrompt, /Never omit FINDING or FETCH STATUS/);
assert.match(systemPrompt, /Never invent values, recommend acquisition steps, use reset --hard, or claim execution/);
